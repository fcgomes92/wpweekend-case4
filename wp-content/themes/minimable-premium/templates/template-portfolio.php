<?php
/**
 * Template Name: One Page - Template Portfolio
 *
 */?>
	<section <?php if (!wp_is_mobile() && get_post_meta($post->ID, 'fw_bg_parallax', true)) {?> data-type="background" data-speed="10"<?php } ?> id="<?php if (get_post_meta($post->ID, 'fw_page_link_alternative', true)) { echo get_post_meta($post->ID, 'fw_page_link_alternative', true); } else { echo the_fw_title(); } ?>" class="section-page <?php echo the_slug(); ?>">
		...
		<div class="container" id="portfolio">
			...
			if ( $count > 0 ){ ?>
				...
				<?php
				if(get_post_meta($post->ID, 'fw_meta_portfolio_link')) {
					$labelLink = get_post_meta($post->ID, 'fw_meta_portfolio_link', true);
				} else {
					$labelLink = $fw_label_portfolio_link;
				}
				if(get_post_meta($post->ID, 'fw_meta_more_link')) {
					$moreLink = $fw_label_more_link;
				}
				?>
				<div class="portfolio-wrapper">
					...
					<div class="row item" data-type="<?php foreach ($categories_portfolio_slug as $key) {
						echo $key->slug . ' ';
						...
						<div class="span4 border description-text">
							<h3><?php the_title(); ?></h3>
							<?php the_excerpt(); ?>
							<?php if (get_post_meta($post->ID, 'fw_enable_more_detail', true)) { ?>
								<a class="link-portfolio open-link" href="<?php the_permalink() ?>">
									<?php if(get_post_meta($post->ID, 'fw_meta_more_link')) {
									echo get_post_meta($post->ID, 'fw_meta_more_link');
									} else {
									echo $moreLink;
									} ?>
								</a>
							<?php } else {?>
								...
								<a class="link-portfolio" href="<?php echo get_post_meta($post->ID, 'fw_link_portfolio', true) ?>">
								<?php echo $labelLink;?></a>
								<?php } ?>
							<?php } ?>
